...
//        VNViewController * vc = [VNViewController new];
//        vc.foobar = dictionary count;
//       // _objcount = vector<int> ids;
        cv::Mat colorMat,  grayImage; //gray1Image,
        cv::cvtColor(mat, colorMat, CV_GRAY2RGB); //cv::COLOR_GRAY2RGB
   //     cv::cvtColor(colorMat, grayImage, CV_RGB2BGR);
    //    cv::cvtColor(gray1Image, grayImage,  CV_BGR2RGBA);// CV_GRAY2RGB
        for (int i=0; i<=ids.size(); i++)
        {
            int id1 = ids[i];
            if (id1 == 2)
            {
                cv::aruco::drawDetectedMarkers(colorMat, corners);
            }
        }
  
        //cv::aruco::drawDetectedMarkers(colorMat, corners);
       // cv::aruco::drawDetectedMarkers(colorMat, corners, ids, cv::Scalar(0,255,24));
        NSUInteger testnumber = corners.size();
        NSUInteger markerID = ids.at(0);
        cv::Mat distCoeffs = cv::Mat::zeros(8, 1, cv::DataType<double>::type); //zero out distortion for now
     

       //MARK: Aruco detection
//        std::vector<cv::Vec3d> rvecs, tvecs
//        cv::aruco::estimatePoseSingleMarkers(corners, marker_dim, intrinMat, distCoeffs, rvec, tvec);
...
//        //MARK: solvepnp
        ...
        std::cout << "corner points: " << corners[0] << std::endl;
      ...
      //  std::cout << "solvePnP points: " << cv::solvePnP << std::endl;
...
//        cv::Point test1 = image_points.at(0);
//        cv::Point test2 = image_points.at(1);
//        cv::Point test3 = image_points.at(2);
//        cv::Point test4 = image_points.at(3);
        if (corners.size() == 4)
        {
            cv::Point test4 = corners[0].at(3);
            cv::Point test3 = corners[1].at(2);
            cv::Point test2 = corners[2].at(1);
            cv::Point test1 = corners[3].at(0);
            NSArray *myArray = @[[NSValue valueWithCGPoint:CGPointMake(test1.x, test1.y)],[NSValue valueWithCGPoint:CGPointMake(test2.x, test2.y)],[NSValue valueWithCGPoint:CGPointMake(test3.x, test3.y)],[NSValue valueWithCGPoint:CGPointMake(test4.x, test4.y)]];
            
            model.testarray = myArray;
        }
